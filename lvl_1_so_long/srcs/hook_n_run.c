...
/*   Updated: 2022/11/27 03:16:04 by nnuno-ca         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	move_player(t_game *game, bool horizontal, int length)
{
	if (horizontal == true)
	{
		if (game->map.map[game->map.player_x][game->map.player_y + length] == WALL)
			return ;
		game->map.player_y += length;
	}
	else
	{
		if (game->map.map[game->map.player_x + length][game->map.player_y] == WALL)
			return ;
