...
/*   Updated: 2022/11/23 23:46:14 by nnuno-ca         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int main(int argc, char **argv)
{
	...
	mlx_ptr = mlx_init();
	win_ptr = mlx_new_window(mlx_ptr, 13 * 64, 5 * 64, "so_long");
	if (!win_ptr)
	...
	mlx_put_image_to_window(mlx_ptr, win_ptr, ylw_ptr, 1 * 32, 1 * 32);
	mlx_put_image_to_window(mlx_ptr, win_ptr, red_ptr, 4 * 32, 1 * 32);
	mlx_put_image_to_window(mlx_ptr, win_ptr, meat_ptr, 7 * 32, 1 * 32);

	mlx_key_hook(win_ptr, on_press, NULL);
	
	// close window on mouse press X
